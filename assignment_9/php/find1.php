...
function get_city($connect, $term){
    $querySingle = "SELECT City FROM House_single WHERE City LIKE '%".$term."%' ORDER BY City ASC";
    $resultSingle = mysqli_query($connect, $querySingle);
    $dataSingle = mysqli_fetch_all($resultSingle, MYSQLI_ASSOC);

    $queryCompany = "SELECT City FROM House_company WHERE City LIKE '%".$term."%' ORDER BY City ASC";
