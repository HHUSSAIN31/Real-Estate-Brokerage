...
def parser_access_log(filename, pagename, pagepath, day):
    with open('../stats/' + filename) as f, open('../stats/' + pagename + '.csv','w') as csvf, open('../stats/' + 'counter.csv','a+') as co:
        log = f.read()
...
def parser_error_log(filename):
    with open('../stats/' + filename) as f, open('../stats/' + 'error.csv','w') as csvf:
        log = f.read()
        regexp = r'\[([A-Za-z]{3} [A-Za-z]{3} \d{2} \d{2}:\d{2}:\d{2}).*(\d{4})\] \[(.*?)\] \[(.*?)\] \[client(.*?)\] [A-Z]{3} [A-Za-z]{6}: (.*?)\bin\b(.*(/~hhussain).*)'
        lists = re.findall(regexp,log)
        #creating csv object file
        writer = csv.writer(csvf)
...
if __name__ == '__main__':
    dict_of_pages = {
        'index': '/~hhussain/assignment_7',
        'login': '/~hhussain/assignment_8/login/login.php',
        'maintenance': '/~hhussain/assignment_5',
        'search': '/~hhussain/assignment_8/search',
        'Imprint': '/~hhussain/assignment_8/imprint.php'
    }
    day='22' #as we didnt have much traffic on other days
